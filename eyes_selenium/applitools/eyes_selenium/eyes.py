...
from eyes_selenium.common.exceptions import WebDriverException
from eyes_selenium.webdriver.remote.webdriver import WebDriver as RemoteWebDriver
...
class Eyes(EyesBase):
    ...
    def __init__(self, server_url=EyesBase.DEFAULT_EYES_SERVER):
        ...
        self._stitch_mode = StitchMode.Scroll  # type: str
    ...
    def base_agent_id(self):
        return "eyes.eyes_selenium.python/{version}".format(version=__version__)
    ...
    def open(self, driver, app_name, test_name, viewport_size=None):
        # type: (AnyWebDriver, str, str, tp.Optional[ViewPort]) -> EyesWebDriver
        if self.is_disabled:
    ...
    def check_window(self, tag=None, match_timeout=-1, target=None):
        # type: (tp.Optional[str], int, tp.Optional[Target]) -> None
        """
    ...
    def check_region(self, region, tag=None, match_timeout=-1, target=None, stitch_content=False):
        # type: (Region, tp.Optional[str], int, tp.Optional[Target], bool) -> None
        """
